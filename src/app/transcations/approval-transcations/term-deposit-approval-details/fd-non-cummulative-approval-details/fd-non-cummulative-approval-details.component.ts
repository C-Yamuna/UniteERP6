    ...
    this.fdNonCummulativeAccountsService.getFdNonCummulativeByBranchIdPacsId(this.pacsId, this.branchId).subscribe((data: any) => {
      this.responseModel = data;
       ...
       obj.statusName != CommonStatusData.CREATED).map((fdNonCummulative: any) => {
         ...
         if (fdNonCummulative.photoCopyPath != null && fdNonCummulative.photoCopyPath != undefined) {
           fdNonCummulative.multipartFileListForPhotoCopy = this.fileUploadService.getFile(fdNonCummulative.photoCopyPath, ERP_TRANSACTION_CONSTANTS.TERMDEPOSITS + ERP_TRANSACTION_CONSTANTS.FILES + "/" + fdNonCummulative.photoCopyPath);
         }
