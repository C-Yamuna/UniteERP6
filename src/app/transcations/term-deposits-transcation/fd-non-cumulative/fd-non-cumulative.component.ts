...
export class FdNonCumulativeComponent implements OnInit {
  ...
  navigateToOperations(event: any,rowData : any) {
    if (event.value === 1)
      this.router.navigate([termdeposittransactionconstant.TERMDEPOSIT_INTEREST_PAYMENT],{ queryParams: { id: this.encryptDecryptService.encrypt(rowData.id) } });
    else if (event.value === 2)
      this.router.navigate([termdeposittransactionconstant.TERMDEPOSIT_FORE_CLOSURE],{ queryParams: { id: this.encryptDecryptService.encrypt(rowData.id) } });
    else if (event.value === 3)
      this.router.navigate([termdeposittransactionconstant.TERMDEPOSIT_CLOSURE],{ queryParams: { id: this.encryptDecryptService.encrypt(rowData.id) } });
    else if (event.value === 4)
      this.router.navigate([termdeposittransactionconstant.TERMDEPOSIT_RENEWAL],{ queryParams: { id: this.encryptDecryptService.encrypt(rowData.id) } });
  }
